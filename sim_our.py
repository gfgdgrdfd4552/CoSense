...
class StreetMonitor:
    ...
    def calculate_parking_positions():
        ...
        # 计算实际可停放车辆数量
        max_vehicles = min(NUM_PARKED_VEHICLES+5, int(road_length / spacing))
...
class VehicleCameraManager:
    ...
    def _parse_image(weak_self, image):
        ...
        if current_time - self.last_capture_time >= CAMERA_FREQ:
            ...
            save_dir = f'output/our/lab3/camera_images/vehicle_{vehicle_id}'
    ...
    def save_angle_data(self, vehicle_id):
        ...
        if len(self.yaw_rate_data) > 0:
            filename = f'output/our/lab3/camera_angles/vehicle_{vehicle_id}.csv'
            os.makedirs('output/our/lab3/camera_angles', exist_ok=True)
...
def spawn_moving_vehicles(world, traffic_manager):
    ...
    for i in range(min(NUM_MOVING_VEHICLES, len(all_spawn_points))):
        ...
        if vehicle is not None:
            ...
            traffic_manager.vehicle_percentage_speed_difference(vehicle, -40)  # 提高速度
...
def main():
    ...
    try:
        ...
        parked_vehicles_started = False  # 标记停放车辆是否已启动

        while time.time() - start_time < SIMULATION_TIME:
            current_time = time.time() - start_time
            
            # 在2分钟后启动停放车辆
            if not parked_vehicles_started and current_time >= 120:
                print("2 minute passed - starting parked vehicles")
                for vehicle in parked_vehicles:
                    if vehicle.is_alive:
                        vehicle.set_autopilot(True)  # 启用自动驾驶
                        # 设置速度比正常快一些
                        traffic_manager.vehicle_percentage_speed_difference(vehicle, -40)
                parked_vehicles_started = True
