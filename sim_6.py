...
class VideoWriterThread(threading.Thread):
    ...
    def __init__(self, vehicle_id):
        ...
        # 图像队列（存储元组：(rgb_frame, depth_frame)）
    ...
    def run(self):
        ...
        while not self.stop_event.is_set() or not self.queue.empty():
            try:
                # 从队列获取图像，最多等待1秒
                rgb_array, depth_array = self.queue.get(timeout=1.0)
                
                # 写入RGB视频
                self.rgb_writer.write(rgb_array)
                
                # 写入深度视频（转换为伪彩色）
                depth_colored = cv2.applyColorMap(cv2.convertScaleAbs(depth_array, alpha=0.03), cv2.COLORMAP_JET)
    ...
    def add_frame(self, rgb_array, depth_array):
        """添加帧到队列"""
        if not self.stop_event.is_set():
            try:
                ...
                # 存储RGB和深度图像（深度图像需要转换为8位）
                depth_8bit = cv2.convertScaleAbs(depth_array, alpha=0.03)
                self.queue.put_nowait((rgb_array.copy(), depth_8bit.copy()))
            except queue.Full:
                pass  # 忽略队列满异常
...
class VehicleRecorder:
    ...
    def record_frame(self, rgb_image, depth_image, vehicle):
        ...
        depth_array = depth_array[:, :, :3]  # 使用前三个通道（RGBA中的RGB）
        
        # 将RGB深度图转换为单通道深度数据
        depth_meters = self._convert_depth_image(depth_array)
        self.last_depth_data = depth_meters  # 缓存深度数据
        
        # 添加帧到视频线程队列
        self.video_thread.add_frame(rgb_array, depth_meters)
    ...
    def _convert_depth_image(self, depth_image):
        """
        将CARLA深度图像（RGB格式）转换为深度值（米）
        深度值存储在RGB三个通道中：depth = (R + G * 256 + B * 256 * 256) / (256 * 256 * 256 - 1) * 1000
        """
        depth_image = depth_image.astype(np.float32)
        
        # 提取RGB通道并归一化
        normalized = (depth_image[:, :, 0] + 
                      depth_image[:, :, 1] * 256 + 
                      depth_image[:, :, 2] * 256 * 256) / (256**3 - 1)
        
        # 转换为米（CARLA深度图范围为0-1000米）
        depth_in_meters = normalized * 1000
...
class VehicleCameraManager:
    ...
    def __init__(self, vehicle):
        ...
        depth_bp.set_attribute('fov', '100')  # 与RGB摄像头相同的视野
        
        # 检查并设置像素格式属性
        if depth_bp.has_attribute('pixel_format'):
            depth_bp.set_attribute('pixel_format', 'PF_Depth')  # 设置深度格式
        elif depth_bp.has_attribute('format'):
            # 某些CARLA版本使用'format'属性
            depth_bp.set_attribute('format', 'PF_Depth')
        else:
            print("⚠️ Depth sensor does not have 'pixel_format' or 'format' attribute")
        ...
        while not self.rgb_queue.empty() and not self.depth_queue.empty():
            ...
            if self.recorder.is_recording:
                ...
                if current_time - self.last_save_time >= self.save_interval:
                    self._save_image_and_metadata(rgb_image, depth_image, vehicle_location)
    ...
    def _save_image_and_metadata(self, rgb_image, depth_image, vehicle_location):
        """保存图片和元数据（包含深度信息），使用相对时间命名"""
        # 获取当前记录器的相对时间
        ...
        try:
            ...
            depth_image.save_to_disk(
                f'{save_dir}/{timestamp_str}_depth.png'
            )
            
            # 保存深度数据（转换为米）
            depth_meters = self.recorder._convert_depth_image(
                np.frombuffer(depth_image.raw_data, dtype=np.dtype("uint8"))
                .reshape(depth_image.height, depth_image.width, 4)[:, :, :3]
            )
            np.save(
                f'{save_dir}/{timestamp_str}_depth.npy',
                depth_meters
            )
            
            print(f"Saved image and depth data at time={timestamp_str}s for sensor vehicle {self.vehicle.id}")
        except Exception as e:
            print(f"Error saving image and metadata: {e}")

def generate_sensor_spawn_points():
    """为感知车辆生成优化的生成点（分布在道路右侧车道，前方有车辆遮挡）"""
    sensor_spawn_points = []
    ...
    for i in range(front_vehicles):
        ...
        print(f"Front sensor spawn point: x={x:.1f}, y={RIGHT_LANE_Y:.1f}")
    ...
    for i in range(middle_vehicles):
        ...
        sensor_spawn_points.append(spawn_point)
        print(f"Middle sensor spawn point: x={x:.1f}, y={RIGHT_LANE_Y:.1f}")
    ...
    for i in range(rear_vehicles):
        ...
        sensor_spawn_points.append(spawn_point)
        print(f"Rear sensor spawn point: x={x:.1f}, y={RIGHT_LANE_Y:.1f}")
    
    print(f"Generated {len(sensor_spawn_points)} optimized sensor spawn points (all in right lane)")
...
def generate_normal_spawn_points():
    ...
    print(f"Generated {len(spawn_points)} normal vehicle spawn points (mostly in left lane)")
    return spawn_points
...
def spawn_moving_vehicles(world, traffic_manager):
    ...
    left_lane_vehicles = int(NUM_NORMAL_VEHICLES * 0.9)  # 90%在左车道
    right_lane_vehicles = NUM_NORMAL_VEHICLES - left_lane_vehicles  # 10%在右车道
    ...
    print(f"  - {sensor_vehicles_spawned} sensor vehicles (RIGHT lane, distributed)")
    print(f"  - {left_spawned} normal vehicles (LEFT lane - congested)")
    print(f"  - {right_spawned} normal vehicles (RIGHT lane - smooth)")
...
def main():
    ...
    try:
        ...
        print(f"Sensor vehicles ({NUM_SENSOR_VEHICLES}) distributed across FRONT, MIDDLE, and REAR regions in RIGHT lane")
        print(f"Normal vehicles ({NUM_NORMAL_VEHICLES}) mostly in LEFT lane (congested)")
        ...
        while time.time() - start_time < SIMULATION_TIME:
            ...
            if time.time() - congestion_start_time > 10:
                ...
                if left_count > 0 and right_count > 0:
                    ...
                    # 如果左车道平均速度低于15km/h，认为拥堵
                    if avg_left_speed < 15 and not is_congested:
                        print(f"🚨 LEFT lane congestion detected! Speed: {avg_left_speed:.1f} km/h")
                        is_congested = True
                    elif avg_left_speed >= 15 and is_congested:
                        print(f"✅ LEFT lane congestion eased. Speed: {avg_left_speed:.1f} km/h")
                        is_congested = False
        ...
        print("\n🏁 Simulation finished")
        print(f"📊 Total LEFT lane congestion duration: {congestion_duration} seconds")
