    ...
    def _save_image_and_metadata(self, rgb_image, depth_image, vehicle_location, target_x):
        ...
        try:
            ...
            rgb_image.save_to_disk(
                f'{save_dir}/x{target_x}_y{vehicle_location.y:.2f}_z{vehicle_location.z:.2f}_rgb.png'
            )
            
            # 保存深度图片（原始格式）
            depth_image.save_to_disk(
                f'{save_dir}/x{target_x}_y{vehicle_location.y:.2f}_z{vehicle_location.z:.2f}_depth.png'
            )
            ...
            np.save(
                f'{save_dir}/x{target_x}_y{vehicle_location.y:.2f}_z{vehicle_location.z:.2f}_depth.npy',
                depth_meters
            )
            
            print(f"Saved image and depth data at x={target_x} for sensor vehicle {self.vehicle.id}")
        except Exception as e:
