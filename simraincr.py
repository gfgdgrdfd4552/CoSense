...
def generate_normal_spawn_points():
    ...
    print(f"Generated {len(spawn_points)} normal vehicle spawn points (mostly in left lane, few in right rear)")
    return spawn_points
...
def spawn_moving_vehicles(world, traffic_manager):
    ...
    for spawn_point in right_lane_points:
        ...
        if vehicle is not None:
            ...
            traffic_manager.distance_to_leading_vehicle(vehicle, 5.0)  # 5.0米安全距离
    ...
    print(f"  - {right_spawned} normal vehicles (RIGHT lane - rear only)")
...
def main():
    ...
    try:
        ...
        print(f"Normal vehicles ({NUM_NORMAL_VEHICLES}) mostly in LEFT lane, few in right rear")
