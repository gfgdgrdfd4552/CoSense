...
NUM_MOVING_VEHICLES = 10  # 运动车辆数量
SIMULATION_TIME = 1200  # 模拟时长(秒)
...
# 道路坐标(单向二车道)
# STREET_START = carla.Location(x=-43.5, y=-40, z=0)  # 道路起点
# STREET_END = carla.Location(x=-43.5, y=0, z=0)  # 道路终点

# STREET_START = carla.Location(x=80, y=15.2, z=0)  # 道路起点
# STREET_END = carla.Location(x=-10, y=15.2, z=0)   # 道路终点

# STREET_START = carla.Location(x=55, y=26.3, z=0)  # 道路起点
# STREET_END = carla.Location(x=83, y=26.3, z=0)   # 道路终点

# STREET_START = carla.Location(x=31, y=26.3, z=0)  # 道路起点
# STREET_END = carla.Location(x=83, y=26.3, z=0)   # 道路终点

# STREET_START = carla.Location(x=-11.5, y=68, z=0)  # 道路起点
# STREET_END = carla.Location(x=44, y=68, z=0)   # 道路终点

# STREET_START = carla.Location(x=24, y=67.8, z=0)  # 道路起点
# STREET_END = carla.Location(x=-11.5, y=67.8, z=0)   # 道路终点

# STREET_START = carla.Location(x=-60, y=15, z=0)  # 道路起点
# STREET_END = carla.Location(x=-90, y=15, z=0)   # 道路终点

STREET_START = carla.Location(x=-30, y=26.3, z=0)  # 道路起点
STREET_END = carla.Location(x=30, y=26.3, z=0)  # 道路终点

# STREET_START = carla.Location(x=-90, y=26, z=0)  # 道路起点
# STREET_END = carla.Location(x=-64, y=26, z=0)   # 道路终点

# STREET_START = carla.Location(x=-50.5, y=-43, z=0)  # 道路起点
# STREET_END = carla.Location(x=-50.5, y=0, z=0)   # 道路终点

####################
# STREET_START = carla.Location(x=104, y=95, z=0)  # 道路起点
# STREET_END = carla.Location(x=104, y=70, z=0)   # 道路终点

# STREET_START = carla.Location(x=60, y=67.8, z=0)  # 道路起点
# STREET_END = carla.Location(x=85, y=67.8, z=0)   # 道路终点

# STREET_START = carla.Location(x=27, y=136, z=0)  # 道路起点
# STREET_END = carla.Location(x=66, y=136, z=0)   # 道路终点

# STREET_START = carla.Location(x=-47, y=77, z=0)  # 道路起点
# STREET_END = carla.Location(x=-47, y=116, z=0)   # 道路终点

# STREET_START = carla.Location(x=-110, y=42, z=0)  # 道路起点
# STREET_END = carla.Location(x=-110, y=80, z=0)   # 道路终点
...
class StreetMonitor:
    ...
    def calculate_parking_positions():
        ...
        # 向右偏移(0.7倍车道宽度)
        right_offset = right_vector * (LANE_WIDTH * 0.7)

        # 计算停车位间距(考虑车长和间隙)
        spacing = VEHICLE_LENGTH * 1.3  # 车辆间保留30%间隙

        # 计算实际可停放车辆数量
        max_vehicles = min(3+NUM_PARKED_VEHICLES, int(road_length / spacing))
    ...
    for i in range(min(NUM_MOVING_VEHICLES, len(all_spawn_points))):
        ...
        if vehicle is not None:
            ...
            vehicle_states[vehicle.id] = {
                ...
                'change_lane_start_time': 0,
                'change_lane_end_time': 0,
                'target_steer': 0
            }
    print(f"Spawned {len(moving_vehicles)} moving vehicles randomly on map")
    return moving_vehicles, vehicle_states

def is_in_right_lane(location):
    """检查是否在右侧车道"""
    # 道路方向向量
    direction = carla.Vector3D(
        x=STREET_END.x - STREET_START.x,
    ...
    if road_length == 0:
        return False
    direction = direction / road_length
    ...
    t = (point_vec.x * direction.x + point_vec.y * direction.y) / road_length
    # 只检测前半段 (t < 0.5)
    if t >= 0.5:
        return False
    # 计算横向偏移（右侧为正）
    right_vector = carla.Vector3D(x=-direction.y, y=direction.x, z=0)
    lateral_offset = (point_vec.x * right_vector.x + point_vec.y * right_vector.y)
    
    # 右侧车道范围
    is_in_lane = (-0.3* LANE_WIDTH <= lateral_offset <= LANE_WIDTH ) 
    
    # 同时确保在道路范围内
    is_on_road = 0 <= t <= 1
    
    return is_on_road and is_in_lane

def is_left_lane_clear(vehicle, world, distance=15.0):
    """检查左侧车道是否畅通"""
    # 获取车辆当前位置和方向
    ...
    # 计算左侧车道位置
    left_vector = transform.get_right_vector() * -1  # 右向量的反方向是左侧
    left_lane_pos = location + left_vector * LANE_WIDTH
    ...
    for other_vehicle in nearby_vehicles:
        ...
        distance_to_other = location.distance(other_loc)
        
        # 检查是否在左侧车道前方
        if distance_to_other < distance:
            # 计算相对位置
            forward_vector = transform.get_forward_vector()
            relative_vector = other_loc - location
            dot_product = forward_vector.x * relative_vector.x + forward_vector.y * relative_vector.y
            
            # 如果在前方且距离较近
            if dot_product > 0 and abs(dot_product) < distance:
                left_vector_to_other = other_loc - left_lane_pos
                if left_vector_to_other.length() < LANE_WIDTH * 0.8:  # 在左侧车道范围内
                    return False
    return True

def calculate_safe_lane_change(vehicle, world):
    """计算安全的变道参数"""
    # 获取当前速度和方向
    velocity = vehicle.get_velocity()
    speed = math.sqrt(velocity.x**2 + velocity.y**2)  # 速度(m/s)
    transform = vehicle.get_transform()
    
    # 根据速度调整转向幅度和持续时间
    if speed < 5:  # 低速时更大幅度转向
        steer = -0.2
        duration = 1
    elif speed < 10:  # 中速
        steer = -0.2
    ...
    # 检查左侧是否有足够空间
    if not is_left_lane_clear(vehicle, world, distance=20.0):
        # 如果左侧不畅通，减小转向幅度并等待
        steer *= 0.5
        duration *= 3.0
    
    return steer, duration

def check_and_change_lane(world, moving_vehicles, vehicle_states):
    """改进的变道逻辑"""
    current_time = time.time()
    ...
    for vehicle in moving_vehicles:
        ...
        if state['is_changing_lane']:
            # 检查是否应该恢复自动驾驶
            if current_time >= state['change_lane_end_time']:
                vehicle.set_autopilot(True)
                vehicle_states[vid]['is_changing_lane'] = False
            else:
                # 应用平滑的转向控制
                progress = (current_time - state['change_lane_start_time']) / \
                          (state['change_lane_end_time'] - state['change_lane_start_time'])
                # 使用正弦曲线平滑转向变化
                current_steer = state['target_steer'] * math.sin(progress * math.pi)
                
                control = vehicle.get_control()
                control.steer = current_steer
                control.throttle = 0.7  # 保持油门
                vehicle.apply_control(control)
            continue
            
        # 如果刚变道过（冷却时间）
        if current_time - state['last_lane_change_time'] < 5.0:
            continue
            
        location = vehicle.get_location()
        
        # 检查是否在目标道路的右侧车道
        if is_in_right_lane(location):
            # 检查左侧车道是否畅通
            if is_left_lane_clear(vehicle, world):
                # 计算安全的变道参数
                steer, duration = calculate_safe_lane_change(vehicle, world)
                
                # 更新状态
                vehicle_states[vid] = {
                    'is_changing_lane': True,
                    'last_lane_change_time': current_time,
                    'change_lane_start_time': current_time,
                    'change_lane_end_time': current_time + duration,
                    'target_steer': steer
                }
                ...
                print(f"Vehicle {vid} starting lane change with steer={steer:.2f} for {duration:.1f}s")
            else:
                # 左侧车道不畅通，减速等待
                control = vehicle.get_control()
                control.throttle = 0.3  # 减速
                control.brake = 0.2  # 轻微刹车
                vehicle.apply_control(control)
...
def main():
    ...
    try:
        ...
        # traffic_manager.global_percentage_speed_difference(-50.0)  # 负值表示加速
        ...
        # 生成运动车辆
        moving_vehicles, vehicle_states = spawn_moving_vehicles(world, traffic_manager)
        ...
        while time.time() - start_time < SIMULATION_TIME:
            ...
            check_and_change_lane(world, moving_vehicles, vehicle_states)
