...
def get_intersection_approach_waypoints(world, target_start):
    """获取所有进入目标道路起点的接近waypoints"""
    # 扩大搜索范围确保覆盖所有进入方向
    search_distance = 10.0  # 米
    approach_waypoints = []
    ...
    for angle in range(0, 360, 45):
        ...
        offset = carla.Location(
            x=search_distance * math.cos(math.radians(angle)),
            y=search_distance * math.sin(math.radians(angle)),
            z=0)
        
        wp = world.get_map().get_waypoint(target_start + offset)
        if wp:
            # 获取这些waypoint的前驱（即连接到它们的waypoint）
            approach_waypoints.extend(wp.previous(5.0))
    
    return list(set(approach_waypoints))  # 去重

def enforce_left_lane_entry(world, traffic_manager):
    """强制车辆只进入左侧车道（0.9.15兼容版）"""
    # 获取目标道路起点waypoint（左侧车道）
    target_left_wp = None
    for offset in [0, -0.5, 0.5, -1.0, 1.0]:  # 尝试不同偏移找到左侧车道
        wp = world.get_map().get_waypoint(
            STREET_START + carla.Location(x=offset, y=offset))
        if wp and wp.lane_id > 0:  # 正lane_id通常是左侧车道
            target_left_wp = wp
            break
    
    if not target_left_wp:
        raise RuntimeError("无法找到目标道路左侧车道")
    ...
    for vehicle in world.get_actors().filter('vehicle.*'):
        ...
        # 检查是否正在接近目标道路
        next_wps = current_wp.next(10.0)  # 查看前方10米路径
        approaching_target = any(
            StreetMonitor.is_on_street(wp.transform.location) 
            for wp in next_wps
        )
        ...
        if approaching_target:
            ...
            try:
                # 方法1：使用速度限制间接控制
                traffic_manager.vehicle_percentage_speed_difference(vehicle, 20)  # 减速
                traffic_manager.ignore_lights_percentage(vehicle, 100)
                
                # 方法2：强制车道保持（0.9.15可用）
                traffic_manager.auto_lane_change(vehicle, False)
                traffic_manager.collision_detection(vehicle, False)
                
                # 方法3：通过目标速度引导
                traffic_manager.set_vehicle_target_velocity(
                    vehicle, 
                    vehicle.get_velocity().length() * 0.9)  # 轻微减速
                
                print(f"已设置车辆 {vehicle.id} 进入左侧车道限制")
            except Exception as e:
                print(f"设置车辆路径时出错: {str(e)}")


def is_right_lane_spawn(spawn_point, world):
    """检查生成点是否在右侧车道"""
    waypoint = world.get_map().get_waypoint(spawn_point.location)
    return waypoint.lane_id < 0  # 负lane_id表示右侧车道
...
def spawn_moving_vehicles(world, traffic_manager):
    ...
    all_spawn_points = [
        ...
        if not is_right_lane_spawn(sp, world)
    ]
    ...
    for i in range(min(NUM_MOVING_VEHICLES, len(all_spawn_points))):
        ...
        if vehicle is not None:
            ...
            # 设置交通管理器参数
            # traffic_manager.ignore_lights_percentage(vehicle, 100)  # 忽略所有交通灯
            traffic_manager.vehicle_percentage_speed_difference(vehicle, -20)  # 提高速度
            traffic_manager.distance_to_leading_vehicle(vehicle, 2.5)  # 保持车距

            # 初始化车辆状态
            vehicle_states[vehicle.id] = {}
    
    print(f"Spawned {len(moving_vehicles)} moving vehicles randomly on map (avoiding right lane)")
...
def main():
    ...
    try:
        ...
        traffic_manager.global_percentage_speed_difference(-20.0)  # 负值表示加速
        # 关键修改：定期执行路径控制
        def update_vehicle_paths():
            """每5秒更新一次车辆路径"""
            enforce_left_lane_entry(world, traffic_manager)
            return True  # 返回True以便重复执行
        
        # 设置定时器
        world.on_tick(lambda _: update_vehicle_paths() if random.random() < 0.2 else None)
        # 生成停放车辆
