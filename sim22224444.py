...
NUM_SENSOR_VEHICLES = 15  # 感知车辆数量减少到10
NUM_NORMAL_VEHICLES = 50   # 普通车辆数量设为0（确保道路空旷）
...
def spawn_moving_vehicles(world, traffic_manager):
    ...
    for spawn_point in all_spawn_points:
        # 选择x坐标在目标道路范围内或附近的生成点
        if 75 <= spawn_point.location.x <= 85 or -15 <= spawn_point.location.x <= 5:
            # 选择y坐标接近目标道路的生成点
            if 10 <= spawn_point.location.y <= 20:
                target_spawn_points.append(spawn_point)
    
    print(f"Found {len(target_spawn_points)} spawn points near target street")
    
    # 如果目标生成点不够，使用所有生成点
    if len(target_spawn_points) < NUM_SENSOR_VEHICLES:
        target_spawn_points = all_spawn_points
        print(f"Using all {len(target_spawn_points)} spawn points")
    
    random.shuffle(target_spawn_points)  # 随机打乱生成点顺序

    # 只生成感知车辆（10辆）
    for i in range(min(NUM_SENSOR_VEHICLES, len(target_spawn_points))):
        ...
        if vehicle is not None:
            ...
            # 设置变道行为（允许变道但频率较低）
            traffic_manager.random_left_lanechange_percentage(vehicle, 0)  # 0%的概率左变道
            traffic_manager.random_right_lanechange_percentage(vehicle, 0)  # 0%的概率右变道
            
            # 安装摄像头
            camera_managers.append(VehicleCameraManager(vehicle))
            print(f"Spawned sensor vehicle {vehicle.id} ({vehicle.type_id}) at ({spawn_point.location.x:.1f}, {spawn_point.location.y:.1f})")
