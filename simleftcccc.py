...
NUM_NORMAL_VEHICLES = 0    # 普通车辆数量（增加到30辆）
SIMULATION_TIME = 3600      # 模拟时长(秒)
IMAGE_WIDTH = 320           # 摄像头图像宽度
IMAGE_HEIGHT = 240         # 摄像头图像高度
...
class VehicleCameraManager:
    ...
    def __init__(self, vehicle):
        ...
        carla.Location(x=2.5, z=0.7)#,  # 位置不变
   # carla.Rotation(pitch=-35, yaw=-20)       # 设置俯仰角-15度使视角朝下
)
...
def generate_sensor_spawn_points():
    ...
    for i in range(front_vehicles):
        ...
        print(f"Front sensor spawn point (LEFT): x={x:.1f}, y={LEFT_LANE_Y:.1f}")
    ...
    for i in range(middle_vehicles):
        ...
        sensor_spawn_points.append(spawn_point)
        print(f"Middle sensor spawn point (LEFT): x={x:.1f}, y={LEFT_LANE_Y:.1f}")
    ...
    for i in range(rear_vehicles):
        ...
        sensor_spawn_points.append(spawn_point)
        print(f"Rear sensor spawn point (LEFT): x={x:.1f}, y={LEFT_LANE_Y:.1f}")
...
def generate_normal_spawn_points():
    ...
    # 1. 左侧车道：在感知车辆前方放置遮挡车辆（5辆）
    # 在道路最前方（x=80）放置遮挡车辆
    for i in range(5):
        x = STREET_START.x - (i * (VEHICLE_LENGTH + 2.5))  # 紧密排列
        spawn_point = carla.Transform(
            carla.Location(x=x, y=LEFT_LANE_Y, z=0.1),
            carla.Rotation(yaw=180)
        )
        spawn_points.append(('LEFT', spawn_point))
        print(f"Added blocking vehicle at x={x:.1f}, y={LEFT_LANE_Y:.1f} (LEFT lane)")
    
    # 2. 右侧车道：密集放置车辆制造拥堵（25辆）
    # 计算需要的间距（紧密排列）
    vehicle_unit_length = VEHICLE_LENGTH + 2.3  # 拥挤交通间距0.3米
    max_vehicles_right_lane = min(25, int(road_length / vehicle_unit_length))
    
    # 在整个右侧车道上密集生成车辆
    for i in range(max_vehicles_right_lane):
    ...
    print(f"Generated {len(spawn_points)} normal vehicle spawn points "
          f"(blockers in LEFT front, congested in RIGHT lane)")
    return spawn_points
...
def spawn_moving_vehicles(world, traffic_manager):
    ...
    for spawn_point in sensor_spawn_points:
        ...
        if vehicle is not None:
            ...
            traffic_manager.distance_to_leading_vehicle(vehicle, 5.0)  # 增加安全距离到5米，确保前方车辆较远
            ...
            print(f"Spawned sensor vehicle {vehicle.id} in LEFT lane, {region} region at x={x_pos:.1f}, y={y_pos:.1f}")
        else:
            print(f"Failed to spawn sensor vehicle at position ({spawn_point.location.x:.2f}, {spawn_point.location.y:.2f})")

    # 生成普通车辆的生成点（左侧车道前方有遮挡车辆，右侧车道拥挤）
    ...
    right_lane_vehicles = min(len(right_lane_points), NUM_NORMAL_VEHICLES)  # 右侧车道尽可能多
    left_lane_vehicles = min(len(left_lane_points), NUM_NORMAL_VEHICLES - right_lane_vehicles)  # 左侧车道放置遮挡车辆
    ...
    for spawn_point in right_lane_points:
        ...
        if vehicle is not None:
            ...
            # 应用油门控制 - 非常慢的速度
            vehicle.apply_control(carla.VehicleControl(
                throttle=0.2,  # 20%油门（非常慢）
                steer=0.0
            ))
            
            # 交通管理器设置
            traffic_manager.vehicle_percentage_speed_difference(vehicle, 0)
            traffic_manager.distance_to_leading_vehicle(vehicle, 3.8)  # 0.8米安全距离（非常近）
    ...
    print(f"  - {left_spawned} normal vehicles (LEFT lane - blockers in front of sensors)")
...
def main():
    ...
    try:
        ...
        traffic_manager.set_global_distance_to_leading_vehicle(3.8)  # 全局安全距离0.8米
        ...
        print(f"Normal vehicles ({NUM_NORMAL_VEHICLES}) - blockers in LEFT front, congested in RIGHT lane")
