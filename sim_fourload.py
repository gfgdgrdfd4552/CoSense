...
NUM_SENSOR_VEHICLES = 25    # 感知车辆数量增加到20辆
NUM_NORMAL_VEHICLES = 100    # 普通车辆数量
...
# 道路坐标(单向二车道)
STREET_START = carla.Location(x=-300, y=30, z=0)  # 道路起点
STREET_END = carla.Location(x=-100, y=30, z=0)   # 道路终点
STREET_WIDTH = 7            # 道路总宽度
VEHICLE_LENGTH = 4.5        # 平均车长(米)
VEHICLE_WIDTH = 2.0         # 平均车宽(米)
VEHICLE_SPACING = 5.0       # 车辆间距(米)

# 违停位置
ILLEGAL_PARKING_POINT = carla.Location(x=-280, y=40, z=10.44)

# 车道中心线定义
LEFT1_LANE_Y = 30 - LANE_WIDTH   # 左车道中心线 y=17.95
LEFT2_LANE_Y = 30    # 左车道中心线 y=17.95
RIGHT1_LANE_Y = 30 + LANE_WIDTH   # 右车道中心线 y=12.45
RIGHT2_LANE_Y = 30 + LANE_WIDTH*2   # 右车道中心线 y=12.45
...
class StreetMonitor:
    ...
    def calculate_parking_positions():
        ...
        for i in range(NUM_PARKED_VEHICLES):
            # 计算每辆车的偏移量
            offset = direction * (i * (VEHICLE_LENGTH + 2.5))
    ...
    def is_on_street(location):
        ...
        # 检查车辆坐标是否在指定范围内
        return (min_x <= location.x <= max_x) and (25 <= location.y <= 40)
...
def generate_sensor_spawn_points():
    ...
    front_section_start = -300
    front_section_end = -100
    front_vehicles_per_lane = 10
    ...
    for lane_name, lane_y in lanes:
        ...
        for i in range(front_vehicles_per_lane):
            ...
            spawn_point = carla.Transform(
                carla.Location(x=x, y=lane_y, z=3),
...
def spawn_parked_vehicles(world):
    ...
    for i, spot in enumerate(parking_spots):
        ...
        if vehicle is not None:
            ...
            vehicle.set_simulate_physics(True)  # 禁用物理模拟
...
def spawn_moving_vehicles(world, traffic_manager):
    ...
    for spawn_point in sensor_spawn_points:
        ...
        if vehicle is not None:
            ...
            if y_pos >= 35:  # 最右侧车道（应急车道）
                lane = "RIGHT2"
            elif 32 <= y_pos < 35:  # 右二车道（行车道）
                lane = "RIGHT1"
            elif 27.5 <= y_pos < 32:  # 右一车道（行车道）
                lane = "LEFT2"
            else:  # 最左侧车道（超车道）
                lane = "LEFT1"
...
def main():
    ...
    try:
        ...
        print(f"Left lane center: y={LEFT1_LANE_Y:.2f}, Right lane center: y={RIGHT1_LANE_Y:.2f}")
