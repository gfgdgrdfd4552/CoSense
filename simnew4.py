...
class VideoWriterThread(threading.Thread):
    ...
    def __init__(self, vehicle_id):
        ...
        video_path = f'{self.save_dir}/recording_{timestamp}.mp4'  # 使用MP4格式
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # 更高效的编码
    ...
    def run(self):
        ...
        while not self.stop_event.is_set() or not self.queue.empty():
            try:
                # 从队列获取图像，最多等待1秒
                array = self.queue.get(timeout=1.0)
    ...
    def add_frame(self, array):
        ...
        if not self.stop_event.is_set():
            try:
                # 如果队列已满，丢弃旧帧
                if self.queue.full():
                    self.queue.get_nowait()  # 丢弃最旧的帧
                self.queue.put_nowait(array.copy())  # 复制数组避免引用问题
            except queue.Full:
                pass  # 忽略队列满异常
...
class VehicleRecorder:
    ...
    def record_frame(self, image, vehicle):
        ...
        # 添加帧到视频线程队列
        self.video_thread.add_frame(array)
...
def spawn_moving_vehicles(world, traffic_manager):
    ...
    for spawn_point in sensor_spawn_points:
        ...
        if vehicle is not None:
            ...
            # 感知车辆设置中等速度（30km/h）并保持近距离
            traffic_manager.vehicle_percentage_speed_difference(vehicle, 0)  # 正常速度
            traffic_manager.set_desired_speed(vehicle, 60)  # 30km/h
            traffic_manager.distance_to_leading_vehicle(vehicle, 1.5)  # 1.5米安全距离
            
            # 严格禁用变道行为
            traffic_manager.random_left_lanechange_percentage(vehicle, 15)
            traffic_manager.random_right_lanechange_percentage(vehicle, 15)
    ...
    for spawn_point in left_lane_points:
        ...
        if vehicle is not None:
            ...
            # 左车道拥挤设置
            speed_setting = random.uniform(60, 70)  # 5-12km/h，拥挤
    ...
    for spawn_point in right_lane_points:
        ...
        if vehicle is not None:
            ...
            # 右车道通畅设置
            speed_setting = random.uniform(60, 70)  # 30-40km/h
            distance_setting = random.uniform(1.0, 5.0)  # 5-10米
