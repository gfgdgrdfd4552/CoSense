...
NUM_NORMAL_VEHICLES = 0    # 普通车辆数量增加以制造拥挤
...
def setup_traffic_light(world):
    ...
    try:
        ...
        if target_light:
            # 设置红绿灯状态为绿灯，持续120秒 [[0]](#__0)
            target_light.set_state(carla.TrafficLightState.Green)
            target_light.set_green_time(120.0)  # 设置绿灯时间为120秒 [[0]](#__0)
...
class StreetMonitor:
    ...
    def is_on_street(location):
        ...
        # 检查y坐标范围（道路宽度内）
        if abs(location.y - STREET_START.y) > STREET_WIDTH / 2:
            return False
            
        return True

    @staticmethod
    def get_lane_from_location(location):
        """根据位置判断车道"""
        if location.y > STREET_START.y:
            return "LEFT"
        else:
            return "RIGHT"
    ...
    def _parse_image(weak_self, image):
        ...
        if on_street:
            if not self.recorder.is_recording and not self.recorder.has_recorded:
                ...
                print(f"Started recording for vehicle {self.vehicle.id} at x={vehicle_location.x:.1f}")
        else:
...
def generate_sensor_spawn_points():
    ...
    for i in range(front_vehicles):
        x = front_section_start - (i * (front_section_start - front_section_end) / front_vehicles) - random.uniform(0, 3)
        spawn_point = carla.Transform(
            carla.Location(x=x, y=LEFT_LANE_Y, z=0.1),
            carla.Rotation(yaw=180)
        )
        sensor_spawn_points.append(spawn_point)
        print(f"Front sensor spawn point: x={x:.1f}")
    
    # 生成中间区域的感知车辆
    for i in range(middle_vehicles):
        x = middle_section_start - (i * (middle_section_start - middle_section_end) / middle_vehicles) - random.uniform(0, 3)
        spawn_point = carla.Transform(
            carla.Location(x=x, y=LEFT_LANE_Y, z=0.1),
            carla.Rotation(yaw=180)
        )
        sensor_spawn_points.append(spawn_point)
        print(f"Middle sensor spawn point: x={x:.1f}")
    
    # 生成后方区域的感知车辆
    for i in range(rear_vehicles):
        x = rear_section_start - (i * (rear_section_start - rear_section_end) / rear_vehicles) - random.uniform(0, 3)
        spawn_point = carla.Transform(
            carla.Location(x=x, y=LEFT_LANE_Y, z=0.1),
            carla.Rotation(yaw=180)
        )
        sensor_spawn_points.append(spawn_point)
        print(f"Rear sensor spawn point: x={x:.1f}")
    
    print(f"Generated {len(sensor_spawn_points)} optimized sensor spawn points")
...
def generate_normal_spawn_points():
    ...
    vehicle_unit_length = VEHICLE_LENGTH + VEHICLE_SPACING  # 5.5米
    ...
    for i in range(max_vehicles_per_lane):
        x = STREET_START.x - (i * vehicle_unit_length) - random.uniform(0, 2)
        spawn_point = carla.Transform(
            carla.Location(x=x, y=LEFT_LANE_Y, z=0.1),
            carla.Rotation(yaw=180)
        )
    ...
    # 为右车道生成密集的生成点（拥挤车辆）
    congested_spacing = 3.0  # 右车道更密集的间距
    max_congested_vehicles = int(road_length / congested_spacing)
    
    for i in range(max_congested_vehicles):
        x = STREET_START.x - (i * congested_spacing) - random.uniform(0, 1)
        spawn_point = carla.Transform(
...
def spawn_moving_vehicles(world, traffic_manager):
    ...
    for spawn_point in sensor_spawn_points:
        ...
        if vehicle is not None:
            ...
            # 感知车辆设置较高速度（相对通畅）
            traffic_manager.vehicle_percentage_speed_difference(vehicle, -20)  # 比限速快20%
            traffic_manager.set_desired_speed(vehicle, 40)  # 35km/h
            traffic_manager.distance_to_leading_vehicle(vehicle, 2.5)  # 2.5米
            ...
            print(f"Spawned sensor vehicle {vehicle.id} in {region} region at x={x_pos:.1f}")
    ...
    right_lane_vehicles = int(NUM_NORMAL_VEHICLES * 0.8)  # 80%在右车道
    left_lane_vehicles = NUM_NORMAL_VEHICLES - right_lane_vehicles  # 20%在左车道
    ...
    for spawn_point in right_lane_points:
        ...
        if vehicle is not None:
            ...
            # 右车道拥挤设置
            speed_setting = random.uniform(8, 15)  # 8-15km/h，更拥挤
            distance_setting = random.uniform(0.3, 0.8)  # 0.3-0.8米，更紧密
    ...
    for spawn_point in left_lane_points:
        ...
        if vehicle is not None:
            ...
            # 左车道通畅设置
            speed_setting = random.uniform(25, 35)  # 25-35km/h
            distance_setting = random.uniform(1.5, 2.5)  # 1.5-2.5米
...
def main():
    ...
    try:
        ...
        print("Sensor vehicles distributed across FRONT, MIDDLE, and REAR regions")
